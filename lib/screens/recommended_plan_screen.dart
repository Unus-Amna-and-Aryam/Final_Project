...
class _RecommendedPlanScreenState extends State<RecommendedPlanScreen> {
  late double _spent;
  List<Providers> _allProviders = [];
  bool _isLoadingProviders = true;
  
  // قائمة لتتبع السعر الحالي لكل بطاقة توصية (لتحديث الحسابات عند تغيير البديل)
  final Map<int, double> _selectedCategoryPrices = {};
  ...
  bool get _needsExceedBudget => _spent > _budget * 1.2 || _spent > _budget;
  ...
  void initState() {
    ...
    _providersFuture.then((providers) {
      ...
      setState(() {
        ...
        _spent = _estimateSpent(providers);
      });
    }).catchError((_) {
      if (!mounted) return;
  ...
  // دالة حساب المصروف مع معالجة الـ null في الأسعار واقتراح أسعار افتراضية واقعية
  double _estimateSpent(List<Providers> allProviders) {
    final selectedNeedsByGroup = <String, List<String>>{};
    for (final needId in widget.answers.selectedNeedsIds) {
      final group = _needIdToCategoryGroup[needId];
    ...
    for (final group in selectedGroups) {
      ...
      for (final needId in selectedNeedsByGroup[group]!) {
        ...
        for (final provider in allProviders) {
          ...
          // معالجة الـ null في الأسعار بوضع قيم افتراضية واقعية ضمن رينج الميزانية
          num? price;
          if (provider.minPrice != null && provider.maxPrice != null) {
            price = (provider.minPrice! + provider.maxPrice!) / 2;
          } else if (provider.minPrice != null) {
            price = provider.minPrice;
          } else if (provider.maxPrice != null) {
            price = provider.maxPrice;
          } else {
            // أسعار افتراضية واقعية إذا كانت القيم null تماماً
            price = (_budget * (normalizedPercent / 100)) * 0.8;
          }
      ...
      double categoryPrice = matchedPrices.isNotEmpty
          ? matchedPrices.reduce((a, b) => a + b) / matchedPrices.length
          : (normalizedPercent / 100) * widget.answers.budget;

      _selectedCategoryPrices[categoryIndex] = categoryPrice.toDouble();
      total += categoryPrice;
      categoryIndex++;
    }
    return total;
  ...
  // تحديث الحسابات عند اختيار بديل جديد
  void _updatePriceForCategory(int categoryIndex, Providers newProvider) {
    num? newPrice;
    if (newProvider.minPrice != null && newProvider.maxPrice != null) {
      newPrice = (newProvider.minPrice! + newProvider.maxPrice!) / 2;
    } else if (newProvider.minPrice != null) {
      newPrice = newProvider.minPrice;
    } else if (newProvider.maxPrice != null) {
      newPrice = newProvider.maxPrice;
    } else {
      newPrice = _budget * 0.1; // سعر افتراضي واقعي عند الـ null
    }

    setState(() {
      _selectedCategoryPrices[categoryIndex] = newPrice!.toDouble();
      // إعادة حساب إجمالي المصروف والمتبقي
      _spent = _selectedCategoryPrices.values.fold(0, (sum, val) => sum + val);
    });
  }
          ...
          children: [
            ...
            for (var i = 0; i < recommendations.length; i++)
              _ServiceCard(
                ...
                onDisplayedProviderChanged: (index, provider) {
                  _displayedPlanProviders![index] = provider;
                  _updatePriceForCategory(index, provider);
                },
              ),
          ],
        );
...
class _ServiceCard extends StatefulWidget {
  ...
  final List<Providers> candidates;
  final void Function(Providers provider) onOpenDetail;
  final void Function(int categoryIndex, Providers provider) onDisplayedProviderChanged;
...
class _ServiceCardState extends State<_ServiceCard> {
  ...
  String get _priceText {
    final price = _provider.minPrice ?? _provider.maxPrice;
    return price != null ? _formatAmount(price) : 'حسب الرينج';
  }

  void _showNextAlternative() {
    setState(() {
      _index = (_index + 1) % widget.candidates.length;
    });
    widget.onDisplayedProviderChanged(widget.categoryIndex, _provider);
  }
